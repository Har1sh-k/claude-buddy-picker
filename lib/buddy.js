...
/**
 * FNV-1a hash (32-bit). Used when Claude Code runs under Node (npm installs).
 *
...
/**
 * Bun.hash (Wyhash) truncated to 32 bits. Used when Claude Code runs as
 * the native binary (which embeds Bun).
 *
 * @param {string} s - The input string.
 * @returns {number} 32-bit unsigned hash.
 */
function bunHash(s) {
  if (typeof Bun === "undefined") {
    throw new Error(
      "Bun.hash is not available. Run this tool under Bun (bun picker.js) " +
      "to brute-force IDs for native Claude Code installs."
    );
  }
...
/**
 * Hash a string to a 32-bit unsigned integer using the appropriate algorithm.
 *
 * @param {string} s - The input string.
 * @param {'wyhash'|'fnv1a'|'auto'} [mode='auto'] - Which hash to use.
 *   'auto' picks based on the current runtime (Bun → wyhash, Node → fnv1a).
 * @returns {number} 32-bit unsigned hash.
 */
function hashString(s, mode = "auto") {
  if (mode === "wyhash") return bunHash(s);
  if (mode === "fnv1a") return fnv1a(s);
  // auto: use runtime detection
  return typeof Bun !== "undefined" ? bunHash(s) : fnv1a(s);
}
